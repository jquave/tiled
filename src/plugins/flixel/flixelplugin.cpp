...
FlixelPlugin::FlixelPlugin()
{
}

Tiled::Map *FlixelPlugin::read(const QString &fileName)
{
    QFile file(fileName);
...
bool FlixelPlugin::supportsFile(const QString &fileName) const
{
    return QFileInfo(fileName).suffix() == QLatin1String("txt");
}

QString FlixelPlugin::nameFilter() const
{
    return tr("Flixel map files (*.txt)");
}

QString FlixelPlugin::errorString() const
{
    return mError;
}

bool FlixelPlugin::write(const Tiled::Map *map, const QString &fileName)
{
  // Only require a collision layer
    /*if (!checkOneLayerWithName(map, QLatin1String("background")))
        return false;
    if (!checkOneLayerWithName(map, QLatin1String("object")))
     return false;*/
    if (!checkOneLayerWithName(map, QLatin1String("detail")))
      return false;
...
bool FlixelPlugin::checkOneLayerWithName(const Tiled::Map *map,
                                        const QString &name)
{
    int count = 0;
